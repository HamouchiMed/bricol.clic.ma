...
const prestatairesData = [
  ...
  { nom: 'Plombier Marrakech 1', metier: 'Plombier', telephone: '0639396740' },
  { nom: 'Plombier Marrakech 2', metier: 'Plombier', telephone: '0622679529' },
  { nom: 'Plombier Marrakech 3', metier: 'Plombier', telephone: '0767148719' },
  { nom: 'Plombier Marrakech 4', metier: 'Plombier', telephone: '0602628460' },
  { nom: 'Plombier Marrakech 5', metier: 'Plombier', telephone: '0600952569' },
  ...
  { nom: 'Climatisation Marrakech 2', metier: 'Climatisation', telephone: '+212767793837' },
...
const seedDatabase = async () => {
  try {
    ...
    await pool.query("INSERT INTO users (phone, email, role) VALUES ('0600000000', 'client@bricol.clic', 'client') ON CONFLICT DO NOTHING");
    await pool.query("INSERT INTO users (phone, email, role) VALUES ('0611111111', 'pro@bricol.clic', 'provider') ON CONFLICT DO NOTHING");
    
    // Seed sample transactions
    console.log('💰 Seeding sample transactions...');
    const client = await pool.query("SELECT id FROM users WHERE role = 'client' LIMIT 1");
    const pro = await pool.query("SELECT id FROM prestataires LIMIT 1");
    
    if (client.rows.length > 0 && pro.rows.length > 0) {
      await pool.query("INSERT INTO transactions (user_id, prestataire_id, title, amount) VALUES ($1, $2, 'Réparation Plomberie', 150.00)", [client.rows[0].id, pro.rows[0].id]);
      await pool.query("INSERT INTO transactions (user_id, prestataire_id, title, amount) VALUES ($1, $2, 'Maintenance Climatisation', 350.00)", [client.rows[0].id, pro.rows[0].id]);
    }
